# k: 95, weight: uniform, p: 2 (euclidean dist)
import pandas as pd
import numpy as np
...
knn.fit(xTrain_G_scaled, yTrain_G)

# NOTE: The original dataset for this script is GERMANY
print("Original Dataset: Germany")

# Make predictions on the test set
yTest_pred = knn.predict(xTest_G_scaled)

# Evaluate accuracy on the test set
selfAcc = accuracy_score(yTest_G, yTest_pred)
print("Self-Test Accuracy:", selfAcc)
# Calculate F1 score on the test set
selfF1 = f1_score(yTest_G, yTest_pred, average='weighted')
print("Test F1 Score:", selfF1)

# Evaluate accuracy on other league test sets
yTest_pred1 = knn.predict(xTest_S_scaled)
...
yTest_pred2 = knn.predict(xTest_I_scaled)
...
yTest_pred3 = knn.predict(xTest_F_scaled)
...
yTest_pred4 = knn.predict(xTest_E_scaled)
acc4 = accuracy_score(yTest_E, yTest_pred4)
print("Test Accuracy England:", acc4)
f1_4 = f1_score(yTest_E, yTest_pred4, average='weighted')
print("Test F1 Score:", f1_4)

# Test Accuracy and F1 Score for other league test sets
data = {
    'Country': ['England', 'Spain', 'Italy', 'France', 'Germany'],
    'Accuracy': [acc4, acc1, acc2, acc3, selfAcc],
...
plt.title('Accuracy vs. F1 Score Matrix (Original Country: Germany)')
plt.show()