...
# This code is very similar to the original data cleanup file; however it only creates one CSV that holds all the leagues,
# and it retains the positionOrder and LeagueID columns to be used when graphing the cluster results
    appearances = pd.read_csv("appearances.csv")

    nonFeatures = ['gameID', 'playerID', 'position', 'positionOrder', 'substituteIn', 'substituteOut']

    feature_to_filter = 'positionOrder'
    value_to_exclude = 17


    # Create a boolean mask for rows to exclude
    mask = appearances[feature_to_filter] == value_to_exclude

    features_to_exclude_template = ['positionOrder', 'leagueID']
    features_to_exclude = appearances[features_to_exclude_template]

    subs = appearances.index[appearances['positionOrder'] == 17].tolist()

    # features_to_exclude = features_to_exclude.drop(features_to_exclude[mask].index)

    # appearances = appearances.drop(appearances[mask].index)

    # appearances = appearances[~mask]

    yTotal = appearances['positionOrder']

    subs = yTotal.index[yTotal == 17].tolist()
    ...
    remaining_features = appearances.drop('positionOrder', axis=1)

    # appearances = appearances.drop(nonFeatures, axis=1)
    ...
    # appearances = appearances[appearances['positionOrder'] != 17]
    ...
    # xWhole = pd.concat([xWhole, yTotal], axis=1)
    ...
    xWhole = pd.concat([xWhole, features_to_exclude], axis=1)

    xWhole = xWhole.drop(index=subs)
    ...
    xWhole.to_csv('xWhole.csv', index=False)
    ...
    # yWhole.to_csv('yWhole.csv', index=False)
